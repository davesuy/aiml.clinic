...
/*
  ...
  @fs_premium_only /inc/global-page.php, /inc/advanced.php, /inc/compatibility/advanced/, /inc/clean-admin-theme/, /assets/vendor/spinkit.min.css, /assets/css/elementor-templates.css
...
if (!class_exists('VG_Admin_To_Frontend')) {
	...
	class VG_Admin_To_Frontend {
		...
		function get_settings($key = null, $default = null, $skip_filter = false) {
			...
			if (!$skip_filter) {
				$out = apply_filters('vg_frontend_admin/settings', $out, $key, $default);
			}
			return $out;
		}
		...
		function init() {
				...
				$all_args = serialize($_GET);
				if (strpos($all_args, 'tb-preview') !== false || strpos($all_args, 'et_fb') !== false || strpos($all_args, 'et-fb') !== false || strpos($all_args, 'cornerstone')) {
					return;
				}
			...
			$extra_files = array_merge($extra_files, $this->get_files_list(__DIR__ . '/inc/compatibility/advanced'));
			
			foreach ($extra_files as $file) {
				require_once $file;
			}
			add_action('transition_post_status', array($this, 'redirect_to_new_after_publish'), 10, 3);
			add_action('transition_post_status', array($this, 'redirect_to_new_after_post_pending_review'), 10, 3);
			add_action('save_post', array($this, 'maybe_redirect_after_post_saved'), 999);
		...
		function redirect_to_login_page_after_wrong_credentials($username = null) {
			...
			if (!empty($referrer) && !strstr($referrer, 'login') && !strstr($referrer, 'wp-admin') && strpos($referrer, 'vgfa_login_failed') === false) {
		...
		function maybe_redirect_after_post_saved() {
			if ($this->redirect_after_save_post) {
				wp_redirect(esc_url($this->redirect_after_save_post));
				exit();
			}
		}
		...
		function get_admin_url_without_base($page_url) {
			...
			if (is_multisite()) {
				foreach (get_sites() as $site) {
					$page_path_only = str_ireplace(get_admin_url($site->blog_id), '', $page_path_only);
				}
			} else {
				$page_path_only = str_ireplace(admin_url(), '', $page_path_only);
			}
		...
		function prepare_loose_url($url) {
			...
			$url = remove_query_arg(array('post', 'token', '_wpnonce', 'user_id', 'wp_http_referer', 's', 'page_id'), $url);
		...
		function get_page_id($admin_url, $title, $required_capability = null) {
			...
			if (!$page_id && ($this->is_master_user() || ($required_capability && current_user_can($required_capability)))) {
				...
				do_action('wp_frontend_admin/frontend_page_created', $page_id, $admin_url, $full_shortcode);
			}
		...
		function render_admin_css($source_id = null) {
			...
			<?php } ?>	
				...
				.woocommerce-page #wpcontent, .woocommerce-page.woocommerce_page_wc-admin #wpbody-content {
					min-height: auto;
				} 
...
// Hide elements selected on quick-settings
			...
			?>				
				...
				.vgca-only-admin-content .edit-post-fullscreen-mode-close:before {
					content: "<";
				}
				...
				/*FluentForms editor*/										
				#ff_form_editor_app .form-editor--sidebar-content {
					height: 500px;
				}

				#ff_form_editor_app .form-editor--body {
					height: 95%;
				}
			</style>
			<script>
		...
		function cleanup_admin_page_for_frontend() {
			...
			$popup_selectors = '.thickbox-loading .media-modal.wp-core-ui,.media-modal, .showwuSweetAlert ';
			...
			$data = apply_filters('vg_admin_to_frontend/backend/js_data', array(
				...
				'links_open_inside_iframe' => array(admin_url(), '&brizy-edit', '&pagebuilder-edit', '#ff_preview'),
			...
			<script id='vg-frontend-admin-backend-init-js-extra'>
				var vgfa_backend_data = <?php echo wp_json_encode($data); ?>;
			</script>
			<script src='<?php echo esc_url(plugins_url('/assets/js/backend.js', VG_Admin_To_Frontend::$file)); ?>?ver=<?php echo filemtime(VG_Admin_To_Frontend::$dir . '/assets/js/backend.js'); ?>' id='vg-frontend-admin-backend-init-js'></script>
