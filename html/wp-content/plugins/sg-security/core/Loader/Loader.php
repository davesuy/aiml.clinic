...
class Loader {
	...
	public function add_usernames_service_hooks() {
		...
		add_action( 'illegal_user_logins', array( $this->usernames_service, 'add_illegal_usernames' ) );
	}
	...
	public function add_sg_2fa_hooks() {
		...
		if ( 0 === intval( get_option( 'sg_security_sg2fa', 0 ) ) ) {
			return;
		}

		add_action( 'wp_login', array( $this->sg_2fa, 'init_2fa' ), 3, 2 );
		add_action( 'login_init', array( $this->sg_2fa, 'validate_2fa_login' ) );
		add_action( 'user_register', array( $this->sg_2fa, 'generate_user_secret' ), 1 );
	}
