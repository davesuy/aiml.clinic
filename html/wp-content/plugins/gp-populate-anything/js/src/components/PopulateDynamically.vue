<template>
	<li :id="'gppa-' + populate" class="gppa field_setting" v-if="isSupportedField">
		...
		<div class="gp-child-settings" v-if="enabled">
			...
			<select class="gppa-object-type" v-model="objectType" @change="changeObjectType">
				...
				<option v-for="objectType in getObjectTypes()" v-bind:value="objectType.id">
					{{ objectType.label }}
				</option>
		...
		computed: {
			...
			templateRows: function () {
				...
				switch (this.populate) {
					...
					case 'values':
						if (this.field.inputs && window.GPPA_ADMIN.interpretedMultiInputFieldTypes.indexOf(this.field.type) === -1) {
