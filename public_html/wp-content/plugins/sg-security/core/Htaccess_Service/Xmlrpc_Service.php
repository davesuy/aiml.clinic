...
class Xmlrpc_Service extends Abstract_Htaccess_Service {
	...
	public $rules = array(
		...
		'disable_all' => '/\#\s+SGS\s+XMLRPC\s+Disable\s+Service\s+<IfModule mod_headers\.c>()\s+<\/IfModule>\s+\#\s+SGS\s+XMLRPC\s+Disable\s+Service\s+END(\n)?/ims',
	);
	...
	public function toggle_rules( $type = 1 ) {
		...
		if ( 1 === $type ) {
			if ( ! empty( $this->plugins_using_xml_rpc() ) ) {
				return false;
			}
			return $this->enable();
		}
